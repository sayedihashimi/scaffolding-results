...
public static class ContactEndpoints
{
    public static void MapContactEndpoints (this IEndpointRouteBuilder routes)
    {
        ...
        group.MapGet("/", async (WebApiEndpoints01Context db) =>
        {
        ...
        group.MapGet("/{id}", async Task<Results<Ok<Contact>, NotFound>> (int id, WebApiEndpoints01Context db) =>
        {
        ...
        group.MapPut("/{id}", async Task<Results<Ok, NotFound>> (int id, Contact contact, WebApiEndpoints01Context db) =>
        {
        ...
        group.MapPost("/", async (Contact contact, WebApiEndpoints01Context db) =>
        {
        ...
        group.MapDelete("/{id}", async Task<Results<Ok, NotFound>> (int id, WebApiEndpoints01Context db) =>
        {
            var affected = await db.Contact
                .Where(model => model.Id == id)
                .ExecuteDeleteAsync();
